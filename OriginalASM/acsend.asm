...
;CT kiểm tra mã số của kí tự có tăng dần hay không
...
.data
	...
	;output strings
	string1 db ' => $'
	string2 db 'has assending digits$'
	string3 db 'has random order or desending digits$'
...
acsending_check: ;checking if ascii code is in desending order 
    mov al, digits[si] ;move digit to al reg

    cmp al, last ;comparing with last digit
    jg No_out ;if it larger than last digit jump to not out
    ...
    jmp acsending_check ;continue the algorithm
